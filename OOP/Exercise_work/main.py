...
# Description: main file for Cowboy vs. Indian game.
...
def main():
    ...
    # Time that it takes to spawn power up randomized
    heart_timer = randint(7, 12) + 3
    super_ammo_timer = randint(14, 20) + 3

    # Timer that is used for games time calculations
    timer = perf_counter()

    # When game restarts timer doesn't reset
    # So timers time needs to be added to power ups.
    if timer > 5:
        heart_timer += timer
    ...
    # Creates indian and cowboy Character instances.
    # Also define graphics to them.
    # Left = left leg animation, Right = right leg
    # X and Y are sprites sizes
    ...
    # Lists for storing all the power ups.
    heart_power_up_list = []
    super_ammo_power_up_list = []

    # Draws each characters hearts into the screen.
    ...
    while running:
        ...
        # Heart power up
        # If enough time has passed creates new power up
        # Timer is reset
